...
- **Toolchain** - a piece of software and its dependencies needed to run on a computer. 
...
The `gizmo` cluster at Fred Hutch actually has 3 head nodes called `rhino` (`rhino01`, `rhino02`, `rhino03`) that are high spec machines (70+ cores, lots of memory). 
...
The filesystem is distributed such that each set of nodes has a relatively fast access to the files on the system. The data itself is *sharded*, or broken up and distributed among the storage servers that provide access. 
